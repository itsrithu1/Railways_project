...
const NewseatAllocationDetailsSchema = Schema(
  {
    train_Number: {
      type: Number,
    },
...
const NewseatAllocation = model("NewseatAllocation", NewseatAllocationDetailsSchema);




async function updateNewseatAllocations() {
  ...
  try {
    const pastNewseatAllocations = await NewseatAllocation.find();

    for (const allocation of pastNewseatAllocations) {
      ...
      if (allocationDate < pastDate) {
        ...
        await NewseatAllocation.deleteOne({ train_Number, name, date });
        ...
        await NewseatAllocation.create({
          train_Number,
          name,
...
cron.schedule("0 0 * * *", () => {
  updateNewseatAllocations();
  console.log("Cron job executed");
});

module.exports.NewseatAllocation = NewseatAllocation;
